...
\maketitle
%\tableofcontents
\subsection*{Talk 1: \href{https://github.com/ramified/personal_handwritten_collection/raw/main/scattered/2026.01.06_talk1_intro_moduliofvb.pdf}{Introduction} (Xiaoxiang Zhou, 2026.01.06)}

\subsection*{Talk 2: Explicit constructions of semistable bundles on elliptic curves (Judikaël Allehou, 2026.01.20, 2026.02.10)}
...
\subsection*{Talk 3: Fourier--Mukai transform on elliptic curve (Aaron Beumer, 2026.02.17-2026.03.03)}
...
\item (Skip this to save time) Sketch the proof of \cite[Theorem 11.4]{PA03} in the case $A$, $B$ are elliptic curves and $S = \{*\}$, i.e., show that 
$$\Phi_K \circ \Phi_{K^{-1}[1]} \cong \Id_{D^b(B)}$$
when the following conditions for $K$ are satisfied: $K \in \Pic(A \times B)$ is a biextension, and the maximal subscheme $S \subseteq B$ satisfying $K_{A \times S} \cong \mathcal{O}_{A \times S}$ is $\{\Id_B\}$. 

\noindent \hspace{-8mm}Assume $A$ is an abelian variety of dimension $n$ from now on.

\item Define the Poincaré line bundle $\mathcal{P} \in \Pic(A \times \hat{A})$ \cite[before Theorem 2.5.1]{BL04} and recall its basic properties \cite[Proposition 2.5.2]{BL04}.
\item Set $\mathcal{S} = \mathcal{S}_A = \Phi_{\mathcal{P}}$. List key properties of $\mathcal{S}$, e.g., \cite[(11.3.1)–(11.3.4), Theorem 11.6, Exercise 7 in p149]{PA03}, and the convolution identity
...
\item Fix a non-degenerate line bundle $\mathcal{L} \in \Pic(A)$. Discuss the $\widetilde{SL}_2(\mathbb{Z})$-action on $D^b(A)$ and describe $\gamma(\mathcal{F})$ for special $\gamma \in \mathrm{SL}_2(\mathbb{Z})$ and $\mathcal{F} \in D^b(A)$ \cite[11.5]{PA03}. Here, $\widetilde{SL}_2(\mathbb{Z})$ is the central extension of $SL_2(\mathbb{Z})$ by $\mathbb{Z}$:
$$\widetilde{SL}_2(\mathbb{Z})= \left< \widetilde{S},\widetilde{T},c\;\textcolor{gray}{ =[-1]}  \;\middle|\; \widetilde{S}^4=2n \cdot c,\;\; \left(\widetilde{T}\widetilde{S}\right)^3=\left(\rule{0mm}{3mm}i(\mathcal{L})+n\right)c \right> $$
% https://q.uiver.app/#q=WzAsNSxbMiwwLCJcXHdpZGV0aWxkZXtTTH1fMihcXG1hdGhiYntafSkiXSxbMywwLCJTTF8yKFxcbWF0aGJie1p9KSJdLFsxLDAsIlxcbWF0aGJie1p9Il0sWzQsMCwiMCJdLFswLDAsIjAiXSxbMiwwXSxbMCwxXSxbMSwzXSxbNCwyXV0=
\[\begin{tikzcd}[ampersand replacement=\&,column sep=2.25em]
	0 \& {\mathbb{Z}} \& {\widetilde{SL}_2(\mathbb{Z})} \& {SL_2(\mathbb{Z})} \& 0
	\arrow[from=1-1, to=1-2]
	\arrow[from=1-2, to=1-3]
	\arrow[from=1-3, to=1-4]
	\arrow[from=1-4, to=1-5]
\end{tikzcd}\]
where $i(\mathcal{L}) \in \mathbb{Z}$ is the integer such that $H^{i(\mathcal{L})}(A;\mathcal{L}) \neq 0$.
\end{itemize}
